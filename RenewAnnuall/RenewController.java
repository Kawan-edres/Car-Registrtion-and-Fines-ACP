...
import useLess.RenewAnnualModel;
...
public class RenewController {
    ...
    public static void addRenewAnnualData() {
        ...
        renewAnnual.add(new RenewAnnualModel(plateNumber, owenerFullName, owenerAddress, carModel, type, carColor, annualStartDate, annualEndDate, vheicleChainNumber, vheicleSafetyEndDate, environmentFinesEndDate, VIN));
    ...
    public static  void delete(int plateNumber){
        readRenewAnnualFromFile();
        renewAnnual.removeIf(p->p.getVehicleNumber() == plateNumber );
        writeRenewAnnualToFile();
    }
   public static void search(int plateNumber){
        readRenewAnnualFromFile();
        renewAnnual.stream().filter(p-> p.getVehicleNumber()==plateNumber ).forEach(p-> System.out.println(p));
